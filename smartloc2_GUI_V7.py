...
APP_NAME = "SmartLoC2 GUI V7"
APP_VERSION = "v7.0.0-concept"
...
def gain_regression_calc(filename, amplitudes_in, period, t_start, pixels_to_plot = [0], debug_level = 0, label = '-'):
    ...
    for pixel_index in range(array_size):
        ...
        gain = round(float(result.slope), 4)
        offset = round(float(result.intercept), 4)
        rsquare = round(float(result.rvalue) ** 2, 6)
...
def perform_measurement(ser, df_in, measurement_configuration, plot_configuration, app = None, cancel_event = None):
    ...
    gain_positive = np.ones(256)
    gain_negative = np.ones(256)
    ...
    # for nsample in range(number_of_samples):
        ...
        if(enable_plot and ((timestamps[nsample]*1e3 % plot_refresh_time_ms) == 0)):
            ...
            selected_pixels_live = get_selected_pixels()
            ...
            root.after(0, lambda ts=timestamps_to_plot, values=np.append(selected_values, array_OF[nsample]): update_plot(ts, values))

            data_to_plot_2D = reshape_data_smartloc2(data_to_plot)
            root.after(0, lambda data2d=data_to_plot_2D, stamp=timestamps[nsample]: update_plot_2D(data2d, f'SmartLoC2 (t = {str(stamp)} s)'))
...
def StartDynamicPlot(container, xmax, Nlines = 1, xlabel = '', ylabel = '', title = '', labels = None):
...
def start_dynamic_plot_2D_plt(container, val_max, val_min, annotate = False, xlabel = '', ylabel = '', title = '', zlabel = ''):
    ...
    ax.set_aspect('equal')
    ax.grid(which='major', color='#ffffff', linewidth=0.6, alpha=0.55)

    data2d = np.zeros((16,16))
    data2d[0][0] = val_max
    data2d[0][1] = val_min

    image = ax.imshow(data2d, cmap='RdBu', interpolation='nearest', vmin=val_min, vmax=val_max)

    if(annotate == True):
        for y in range(16):
            for x in range(16):
                ax.text(x, y, f'{int(16*x+y)}',
...
active_plot = True
MAX_PLOTTED_PIXELS = 8
selected_pixels = [1, 2, 3, 4, 5, 6, 7, 8]
ground_truth_pixels = []
save_measurement_var = tk.BooleanVar(value=True)
selection_status_var = tk.StringVar(value="Selected pixels: 1, 2, 3, 4, 5, 6, 7, 8")
ground_truth_var = tk.StringVar(value="")
serial_port_combo = None
selected_overlay_patches = []
ground_truth_artist = None
line_plot_container = None
map_plot_container = None
console_text = None
selected_pixel_marker_color = "#D98C2B"
...
def normalize_selected_pixels(pixel_values):
    unique_pixels = []
    for pixel in pixel_values:
        try:
            ...
            continue
        if 0 <= pixel_index < 256 and pixel_index not in unique_pixels:
            unique_pixels.append(pixel_index)
    return unique_pixels[:MAX_PLOTTED_PIXELS]


def get_selected_pixels():
    return normalize_selected_pixels(selected_pixels)


def refresh_selected_pixels_label():
    if selection_status_var is None:
        return

    pixels = get_selected_pixels()
    if pixels:
        selection_status_var.set("Selected pixels: " + ", ".join(str(pixel) for pixel in pixels))
    else:
        selection_status_var.set("Selected pixels: none")


def pixel_index_to_xy(pixel_index):
    return divmod(int(pixel_index), 16)
...
def parse_pixel_list(raw_value):
    ...
    for token in normalized.split(","):
        ...
        try:
            ...
            continue
        if 0 <= pixel_index < 256 and pixel_index not in parsed_pixels:
            parsed_pixels.append(pixel_index)
    return parsed_pixels


def set_ground_truth_pixels(pixel_values):
    global ground_truth_pixels
    ground_truth_pixels = parse_pixel_list(pixel_values)
    refresh_ground_truth_overlay()


def refresh_ground_truth_overlay():
    global ground_truth_artist, ax_2D, canvas_2D

    if ax_2D is None or canvas_2D is None:
        return

    ground_truth_xy = np.array([pixel_index_to_xy(pixel_index) for pixel_index in ground_truth_pixels], dtype=float)

    if ground_truth_artist is None:
        ground_truth_artist = ax_2D.scatter([], [], s=120, marker='o', c='#1E2D27', edgecolors='white', linewidths=1.0, zorder=5)

    if ground_truth_xy.size == 0:
        ground_truth_artist.set_offsets(np.empty((0, 2)))
    else:
        ground_truth_artist.set_offsets(ground_truth_xy)

    canvas_2D.draw_idle()


def refresh_selection_overlay():
    global selected_overlay_patches, ax_2D, canvas_2D

    if ax_2D is None or canvas_2D is None:
        return

    for patch in selected_overlay_patches:
        try:
            patch.remove()
        except ValueError:
            pass
    ...
    for pixel_index in pixels:
        x_value, y_value = pixel_index_to_xy(pixel_index)
        patch = Rectangle((x_value - 0.5, y_value - 0.5), 1, 1, facecolor=(0, 0, 0, 0.12), edgecolor='none', zorder=4)
        ax_2D.add_patch(patch)
        selected_overlay_patches.append(patch)

    canvas_2D.draw_idle()


def refresh_line_labels_and_reset():
    global fig, ax, lines, canvas_lines

    if fig is None or ax is None or lines is None or canvas_lines is None:
        return
    ...
    labels = [f"pix {pixel_index}" if pixel_index is not None else "_nolegend_" for pixel_index in pixels] + ["OF"]
    ...
    ax.autoscale_view()
    ax.legend(ncols=2)
    canvas_lines.draw_idle()


def set_selected_pixels(pixel_values):
    global selected_pixels
    selected_pixels = normalize_selected_pixels(pixel_values)
    refresh_selected_pixels_label()
    refresh_selection_overlay()
    refresh_line_labels_and_reset()
...
def on_map_click(event):
    ...
    x = int(np.clip(round(event.xdata), 0, 15))
    y = int(np.clip(round(event.ydata), 0, 15))
    pixel_index = 16 * x + y

    current_pixels = get_selected_pixels()
    if pixel_index in current_pixels:
        current_pixels.remove(pixel_index)
        log_message(f"Pixel {pixel_index} removed from line plot selection")
    else:
        if len(current_pixels) >= MAX_PLOTTED_PIXELS:
            log_message(f"Selection limited to {MAX_PLOTTED_PIXELS} pixels")
            return
        current_pixels.append(pixel_index)
        log_message(f"Pixel {pixel_index} added to line plot selection")

    set_selected_pixels(current_pixels)
...
style.configure(".", font=default_font, background=BG_MAIN)
style.configure("TFrame", background=BG_MAIN)
style.configure("TLabel", padding=(2, 2), background=BG_MAIN, foreground=FG_TEXT)
style.configure("TEntry", padding=(5, 4))
style.configure("TButton", padding=(8, 5))
style.configure("TRadiobutton", padding=(2, 2), background=BG_PANEL, foreground=FG_TEXT)
style.configure("TCheckbutton", padding=(2, 2), background=BG_PANEL, foreground=FG_TEXT)
style.configure("TLabelframe", padding=10, background=BG_PANEL)
style.configure("TLabelframe.Label", padding=(4, 0), background=BG_PANEL, foreground=GREEN_DARK, font=(default_font[0], 10, "bold"))
style.configure("TCombobox", padding=4)
style.map("TButton", foreground=[("disabled", "#A9A9A9")], background=[("active", GREEN_SOFT)])
...
dashboard = ttk.Frame(root, padding=10)
dashboard.grid(row=0, column=0, columnspan=2, rowspan=2, sticky="nsew")
dashboard.columnconfigure(0, weight=1, uniform="left")
dashboard.columnconfigure(1, weight=2, uniform="right")
dashboard.rowconfigure(0, weight=1)
dashboard.rowconfigure(1, weight=1)

config_frame = ttk.LabelFrame(dashboard, text="CONFIGURATION", padding=10)
config_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 8), pady=(0, 8))
config_frame.columnconfigure(0, weight=1)

lf_meas = ttk.LabelFrame(config_frame, text="MEASUREMENT", padding=10)
lf_meas.grid(row=0, column=0, sticky="ew")
lf_meas.columnconfigure(1, weight=1)

meas_param1 = tk.StringVar(value="900")
...
ttk.Checkbutton(lf_meas, text="Store measurement to file", variable=save_measurement_var).grid(row=1, column=0, columnspan=2, sticky="w", pady=3)
ttk.Label(lf_meas, text="Disable high-pass filter at [s]:").grid(row=2, column=0, sticky="w", pady=3)
ttk.Entry(lf_meas, textvariable=meas_param2).grid(row=2, column=1, sticky="ew", pady=3)
ttk.Label(lf_meas, text="Sampling period [ms]:").grid(row=3, column=0, sticky="w", pady=3)
ttk.Entry(lf_meas, textvariable=meas_param3).grid(row=3, column=1, sticky="ew", pady=3)
ttk.Label(lf_meas, text="File to store the data:").grid(row=4, column=0, sticky="w", padx=(0, 8))

default_name = "filename"
filename_var = tk.StringVar(value=default_name)
ttk.Entry(lf_meas, textvariable=filename_var).grid(row=4, column=1, sticky="ew")

do_gain_calib = tk.IntVar()
ttk.Checkbutton(lf_meas, text="Run gain calibration", variable=do_gain_calib).grid(row=5, column=0, columnspan=2, sticky="w", pady=3)

apply_gain_calib = tk.IntVar()
ttk.Checkbutton(lf_meas, text="Apply gain calibration", variable=apply_gain_calib).grid(row=6, column=0, columnspan=2, sticky="w", pady=3)

ttk.Label(lf_meas, text="Gain calibration file:").grid(row=7, column=0, sticky="w", padx=(0, 8))
...
ttk.Entry(row_frame, textvariable=filename_gain_var).grid(row=0, column=0, sticky="ew")
...
ttk.Button(row_frame, text="...", command=browse_file).grid(row=0, column=1, sticky='e')

lf_plot = ttk.LabelFrame(config_frame, text="PLOT SETTINGS", padding=10)
lf_plot.grid(row=1, column=0, sticky="ew", pady=(8, 0))
...
nrow += 1
selection_status_var = tk.StringVar(value="Selected pixels: " + ", ".join(str(pixel) for pixel in selected_pixels))
ttk.Label(lf_plot, textvariable=selection_status_var, wraplength=430, foreground=GREEN_DARK).grid(row=nrow, column=0, columnspan=2, sticky="w", pady=(6, 0))

nrow += 1
ttk.Label(lf_plot, text="Ground truth pixels / wells:").grid(row=nrow, column=0, sticky="w", pady=(8, 3))
ttk.Entry(lf_plot, textvariable=ground_truth_var).grid(row=nrow, column=1, sticky="ew", pady=(8, 3))

nrow += 1
ttk.Label(lf_plot, text="Comma separated pixel numbers.").grid(row=nrow, column=0, columnspan=2, sticky="w")

def _sync_ground_truth(*_args):
    set_ground_truth_pixels(ground_truth_var.get())

ground_truth_var.trace_add("write", _sync_ground_truth)

control_frame = ttk.LabelFrame(config_frame, text="CONNECTION / ACTIONS", padding=10)
...
list_btn = tk.Button(control_frame, text="refresh PORTS", fg="white", bg=BLUE, activebackground="#245A6B", height=2)
list_btn.grid(row=0, column=0, sticky="ew", padx=(0, 8))

input_serial_port = tk.StringVar(value="")
ttk.Label(control_frame, text="Serial port:").grid(row=0, column=1, sticky="e", padx=(0, 6))
serial_port_combo = ttk.Combobox(control_frame, textvariable=input_serial_port, state="readonly")
serial_port_combo.grid(row=0, column=2, sticky="ew", padx=(0, 8))

run_btn = tk.Button(control_frame, text="RUN", fg="white", bg=GREEN, activebackground="#17523A", height=2)
run_btn.grid(row=1, column=0, sticky="ew", padx=(0, 8), pady=(8, 0))

finish_btn = tk.Button(control_frame, text="FINISH", fg="white", bg=RED, activebackground="#91322B", height=2)
finish_btn.grid(row=1, column=1, sticky="ew", padx=(0, 8), pady=(8, 0))

plot_btn = tk.Button(control_frame, text="stop PLOT", fg="white", bg=GOLD, activebackground="#9A651D", height=2)
plot_btn.grid(row=1, column=2, sticky="ew", padx=(0, 8), pady=(8, 0))

dashboard.columnconfigure(0, weight=1)
dashboard.columnconfigure(1, weight=2)

console_frame = ttk.LabelFrame(dashboard, text="CONSOLE", padding=(10, 6))
console_frame.grid(row=1, column=0, sticky="nsew", padx=(0, 8), pady=(0, 0))
console_frame.columnconfigure(0, weight=1)
console_frame.rowconfigure(0, weight=1)
console_frame.grid_propagate(False)
console_frame.configure(height=165)

log_text = tk.Text(console_frame, height=8, wrap="word", state="disabled")
log_text.grid(row=0, column=0, sticky="nsew")
log_scroll = ttk.Scrollbar(console_frame, orient="vertical", command=log_text.yview)
log_scroll.grid(row=0, column=1, sticky="ns")
log_text.configure(yscrollcommand=log_scroll.set)

line_plot_container = ttk.LabelFrame(dashboard, text="LINE PLOTS", padding=10)
line_plot_container.grid(row=0, column=1, sticky="nsew", pady=(0, 8))
line_plot_container.columnconfigure(0, weight=1)
line_plot_container.rowconfigure(0, weight=1)

map_plot_container = ttk.LabelFrame(dashboard, text="2D MAP", padding=10)
map_plot_container.grid(row=1, column=1, sticky="nsew")
map_plot_container.columnconfigure(0, weight=1)
map_plot_container.rowconfigure(0, weight=1)

def initialize_dashboard_preview():
    preview_measurement_configuration = {
        "measurement_time_s": int(meas_param1.get()),
        "apply_gain_calib": bool(apply_gain_calib.get()),
    }
    preview_plot_configuration = {
        "line_ylim": int(plot_param3.get()),
        "map2D_maxval": int(plot_param4.get()),
        "map2D_minval": -int(plot_param4.get()),
    }
    start_plot(preview_measurement_configuration, preview_plot_configuration)

root.after(50, initialize_dashboard_preview)
        ...
        if df_out is not None:
            if save_measurement or measurement_configuration.get('do_gain_calib'):
                ...
                if not save_measurement and measurement_configuration.get('do_gain_calib'):
                    ...
                    log_message(f'File exists! Saving as "{target_file}" ...')

                df_out.to_pickle(target_file + extension)
                log_message(f'"{target_file + extension}" stored in disk')
            else:
                log_message('Measurement file save disabled by user.')
...
def on_list_ports():
    refresh_serial_ports(manual=True)


def refresh_serial_ports(manual=False):
    global port_values, serial_port_combo

    ports = list(serial.tools.list_ports.comports())
    port_values = [p.device for p in ports]

    if serial_port_combo is not None:
        serial_port_combo.configure(values=port_values)

        current_value = input_serial_port.get().strip()
        if port_values:
            if current_value not in port_values:
                input_serial_port.set(port_values[0])
        else:
            input_serial_port.set("")
    ...
    if not manual:
        root.after(2000, refresh_serial_ports)
...
def on_run():
    ...
    measurement_configuration = dict(
        ...
        save_measurement        = save_measurement_var.get()
        )  


    plot_configuration = dict(
        pixels_to_plot          = get_selected_pixels(),
    ...
    serial_port = input_serial_port.get().strip()

    if not serial_port:
        log_message("No serial port selected.")
        running = False
        run_btn.config(state="normal")
        finish_btn.config(state="disabled")
        return
...
def start_plot(measurement_configuration,plot_configuration):
    ...
    global ground_truth_artist, selected_overlay_patches
    ...
    fig, ax, lines, canvas_lines = StartDynamicPlot(
        line_plot_container,
        measurement_time_s,
        Nlines = MAX_PLOTTED_PIXELS + 1,
        xlabel = 'time [s]',
        ylabel = ylabel,
        title = 'SmartLoC2',
    ...
    fig_2D, image_2D, ax_2D, canvas_2D = start_dynamic_plot_2D_plt(map_plot_container, val_max = map2D_maxval, val_min = map2D_minval, annotate = True, title = 'SmartLoC2', 
                                                        zlabel = ylabel)

    global map_click_cid
    refresh_selection_overlay()
    refresh_selected_pixels_label()
    refresh_ground_truth_overlay()
    map_click_cid = canvas_2D.mpl_connect('button_press_event', on_map_click)
