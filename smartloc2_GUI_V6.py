...
def perform_measurement(ser, df_in, measurement_configuration, plot_configuration, app = None, cancel_event = None):
    ...
    array_OF_plot       = np.zeros((number_of_samples))
    ...
    # for nsample in range(number_of_samples):
        ...
        array_OF[nsample] = data_overflow
        array_OF_plot[nsample] = 0    # do not show overflow in the plot
            
        # Remove outliers (have yet to find out why they are produced)
        for npixel,events in enumerate(array_events[nsample]):
            if ((events < -outlier_threshold_events) or (events > outlier_threshold_events)):
                array_events[nsample, npixel] =  array_events[nsample-1, npixel]
        ...
        if(enable_plot and ((timestamps[nsample]*1e3 % plot_refresh_time_ms) == 0)):
            ...
            if(plot_type in ['line','both']): 
                ...
                root.after(0, lambda: update_plot(timestamps_to_plot, np.append(array_OF_plot[nsample],data_to_plot[pixels_to_plot])) )
...
plot_param1 = tk.StringVar(value="1,2,3,4,5,6,7,8")
plot_param2 = tk.StringVar(value="500")
plot_param3 = tk.StringVar(value="200")
plot_param4 = tk.StringVar(value="200")
...
def start_plot(measurement_configuration,plot_configuration):
    ...
    if(plot_type in ['line','both']):         
        ...
        fig, ax, lines, canvas_lines = StartDynamicPlot(measurement_time_s, Nlines = plot_Nlines, xlabel = 'time [s]', ylabel = ylabel,
                                                  title = 'SmartLoC2', labels = ['OF'] + [f'pix{n}' for n in pixels_to_plot]) 
        if(line_ylim != None):
